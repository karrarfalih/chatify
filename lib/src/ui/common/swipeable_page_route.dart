...
class _DirectionDependentDragGestureRecognizer
  ...
  bool _shouldHandle(PointerEvent event) {
    ...
    final isCorrectDirection =
        ((directionality == TextDirection.ltr && event.delta.dx > 0) ||
            (directionality == TextDirection.rtl && event.delta.dx < 0) ||
            event.delta.dx == 0);
...
class _DetectionArea {
  final double startOffset, width;

  _DetectionArea({required this.startOffset, required this.width});
}
