...
class _MessagesSelectionListenerState extends State<MessagesSelectionListener> {
  ...
  _detectTapedItem(PointerEvent event) {
    if (!context.read<SelectionBloc>().state.isSelectionMode) return;
    ...
    if (box.hitTest(result, position: local)) {
      for (final hit in result.path) {
        ...
        if (target is SelectedMessage) {
          final initialList = Map<String, Message>.from(
            bloc.state.selectedMessages,
          );
          final selectedMessages = Map<String, Message>.from(
            bloc.state.selectedMessages,
          );
          ...
          if (isRemove) {
            ...
            selectedMessages.addAll(
              addedMessages.map(
                (key, value) => MapEntry(value.content.id, value),
              ),
            );
            selectedMessages.addAll(initialSelecetdMessages);
          }
          if (initialList.length != selectedMessages.length) {
            bloc.add(SelectionChanged(selectedMessages));
          }
        }
      }
    }
  ...
  _detectStart(PointerEvent event) {
    addedMessages.clear();
    initialSelecetdMessages = context
        .read<SelectionBloc>()
        .state
        .selectedMessages;
    final RenderBox box = _key.currentContext!
    ...
    if (box.hitTest(result, position: offset)) {
      for (final hit in result.path) {
        final target = hit.target;
        if (target is SelectedMessage) {
          isRemove = initialSelecetdMessages.containsKey(
            target.message.content.id,
          );
        }
      }
    }
